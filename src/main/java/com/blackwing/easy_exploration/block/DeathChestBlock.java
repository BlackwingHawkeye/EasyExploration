package com.blackwing.easy_exploration.block;
...
public class DeathChestBlock extends Block {
	...
	public DeathChestBlock(final Properties properties) {
		super(properties);
	}
	...
	public TileEntity createTileEntity(final BlockState state, final IBlockReader world) {
		...
		return ModTileEntityTypes.DEATH_CHEST.get().create();
	}
	...
	public void onReplaced(BlockState oldState, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		if (oldState.getBlock() != newState.getBlock()) {
			...
			if (tileEntity instanceof DeathChestTileEntity) {
				final ItemStackHandler inventory = ((DeathChestTileEntity) tileEntity).inventory;
	...
	public ActionResultType onBlockActivated(final BlockState state, final World worldIn, final BlockPos pos, final PlayerEntity player, final Hand handIn, final BlockRayTraceResult hit) {
		if (!worldIn.isRemote) {
			...
			if (tileEntity instanceof DeathChestTileEntity)
				NetworkHooks.openGui((ServerPlayerEntity) player, (DeathChestTileEntity) tileEntity, pos);
		}
